...
import coo.mvc.security.model.PwdChangeModel;
...
public class PersonAction {
   ...
   * 保存个人密码。
   * 
   * @param pwdChangeModel 修改密码表单Model
   * @return 返回保存个人密码提示信息。
   */
  @RequestMapping("person-pwd-change-save")
  public ModelAndView pwdChangeSave(PwdChangeModel pwdChangeModel) {
    securityService.changePassword(pwdChangeModel.getOldPwd(), pwdChangeModel.getNewPwd());
    return new DwzResultBuild().success("person.pwd.change.success").closeDialog().build();
  }
